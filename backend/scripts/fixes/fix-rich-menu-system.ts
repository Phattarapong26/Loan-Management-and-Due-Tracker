...
const RICH_MENU_CONFIGS: Record<string, RichMenuConfig> = {
    USER: {
    ...
    ADMIN: {
        ...
        areas: [
            ...
            { bounds: { x: 1250, y: 1124, width: 1250, height: 562 }, action: { type: 'message', label: 'เมนูคำสั่ง', text: 'เมนू' } },
        ],
    },
};
...
async function createRichMenu(role: string, config: RichMenuConfig): Promise<string | null> {
...
async function uploadRichMenuImage(richMenuId: string, role: string): Promise<boolean> {
...
async function storeRichMenuId(role: string, richMenuId: string): Promise<void> {
...
async function assignRichMenuToUsers(): Promise<void> {
    ...
    for (const user of users) {
        ...
        try {
            ...
            console.log(`✅ Assigned ${user.role} Rich Menu to user ${user.id} (${user.lineUserId})`);
        } catch (error: any) {
    ...
    try {
        ...
        for (const [role, config] of Object.entries(RICH_MENU_CONFIGS)) {
            ...
            if (richMenuId) {
                ...
                // Step 3: Upload image for Rich Menu
