...
interface LoanProductStatsCardsProps {
  total: number;
  active: number;
  inactive: number;
  popular: number;
...
export function LoanProductStatsCards({
  total,
  active,
  inactive,
  popular,
  isLoading = false,
}: LoanProductStatsCardsProps) {
  const cards = [
    {
    ...
    {
      title: 'ไม่ใช้งาน',
      value: formatCompactNumber(inactive),
      unit: 'รายการ',
      icon: Package,
    ...
    {
      title: 'สินเชื่อยอดนิยม',
      value: formatCompactNumber(popular),
      unit: 'รายการ',
      icon: Star,
